...
function Panier({ navigation , route}) {
    ...
    let fetchData = async()=>{
        setload(true)
        await axios.get(`http://10.0.2.2:3001/api/users/${userName}/cart`).then(res=>{
      setPanier(res.data);
        }).catch(err=>{
          console.log(err);
        })
...
let PasserCommande = async(totalPrice,totalQuantity)=>{
    ...
    let newSold=0;
    let cur='';
    await axios.get(`http://10.0.2.2:3001/api/users/${userName}`).then(res=>{
   ...
   console.log(userSold);
    listPanier.cart && listPanier.cart.items && Object.keys(listPanier.cart.items).map(async (id) =>{
      let data1={
       ...
       await axios.post(`http://10.0.2.2:3001/api/UpdateDevises`,data1).then(res=>{
            ...
            if(userSold > 0){
                if(listPanier.cart.items[id].produit.cur === '$'){
                 ...
                 addNewCommande(userName,totalPrice,totalQuantity,email,cur,newSold);
                     ...
                     newSold= parseFloat(userSold - totalPrice*1.4).toFixed(2);
                     //ADD new Commande
                 addNewCommande(userName,totalPrice,totalQuantity,email,cur,newSold);
                     
                 }else if(listPanier.cart.items[id].produit.cur === 'MAD'){
                     newSold= parseFloat(userSold - totalPrice/10).toFixed(2);
                     //ADD new Commande
                     addNewCommande(userName,totalPrice,totalQuantity,email,cur,newSold);
                 }
              }else{
...
await axios.post(`http://10.0.2.2:3001/api/addCommande`,data).then(res=>{
  let data = {
      fullName:userName,
        newSold:newSold,
        fullName:userName
  };
   axios.post(`http://10.0.2.2:3001/api/updateSold`,data).then(res=>{
      console.log(res);
